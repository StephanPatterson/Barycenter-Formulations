...
int main(int argc, const char * argv[]) {
   ...
   int startyear = 13;
   std::string startmonth = "Nov";
   int Pnum =15;//If NumMoMeas = 1, Number of months to process. Will go consecutively from start month, skipping any empty months.
   ...
   if (dotoy ==1)
   {
      ...
      lambda[Pnum-1] = 1-sum;
      //   std::cout << lambda[Pnum-1] << std::endl;
   }
   else
   {
      ...
      for (int i = 0; i < Pnum-2; ++i)
      {
         ...
         for (int j = 0; j < Psnum[i]-1; ++j)
         {
            ...
            //         std::cout <<Psupp[startindices[i]+j] << std::endl;
         }
         Psupp[totalsupp-1].mass = 1-totalmass;
         //      std::cout <<Psupp[startindices[i]+Psnum[i]-1] << std::endl;
         currentmonth = month;
      ...
      if (comptoTrue == 1)
      {
         ...
         fname3 << "/Users/spatterson/Documents/Column Generation/ColGen2ARandomYr" << startyear << "_" << Pnum << ".txt";
...
/*   for (int i = 0; i < lamk.size(); ++i)
   {
      std::cout << lamk[i].get(GRB_DoubleAttr_X) << std::endl;
   }
   */
   if (comptoTrue == 1)
   {
...
/*   std::vector<double> yhatAmas2(blockreplength,0);
   adjusttotals(yhatAmas2, yhat, Amrn, blockreplength,Psnum,Pnum);
   for (int i = 0; i < blockreplength; ++i)
   {
      if (yhatAmas[i] - yhatAmas2[i] != 0)
      {
...
//   std::cout <<numuniquecol <<std::endl;
